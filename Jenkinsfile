pipeline {
  ...
  stages {
    ...
    stage('Get Performance Testing Repo') {
      steps {
        echo "Getting the Performance Testing Repo"
        git(
          url: 'git@github.com:MorielMauni/petclinic-performance.git',
      ...
      steps {
        ...
        dir('./performance-test') {
          ...
          script {
            ...
            // Update the path to use the correct JMeter installation
            sh '/opt/jmeter/bin/jmeter -n -t ./test-plan.jmx -l ./test-results.csv'
            sh 'mkdir html-report'
            sh 'ls -l'
            echo "create html report"
            sh '/opt/jmeter/bin/jmeter -g ./test-results.csv -e -o html-report'
